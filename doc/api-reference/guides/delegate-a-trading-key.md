...
* **Funds stay with the master.** A delegate cannot withdraw: withdrawals are a [bridge precompile](../applications-precompiles/bridge.md) call with no delegation envelope, signed only by the master — so a leaked delegate key can trade the balance but never move it off Pod.
