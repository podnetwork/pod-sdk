...
* **Delegated calls pay no gas**, which is what lets a session key withdraw an orderbook balance without holding a native Pod balance — a direct caller needs one, and `withdraw` no longer credits it.
