...
Applications can subscribe to a **time of interest**  - for example, an auction deadline  - using `pod_subscribe`. Once a full node determines that the subscribed time has been reached by the network, it returns a **past perfect set** associated with that timestamp.

The past perfect set provides three properties:
