...
mark_price     = clamp(oracle_price + price_diff_ema, last_mark_price, mark_price_clamp × last_mark_price)
```
...
- `mark_price_clamp` - a per-market bound limiting how far `mark_price` can move from the previous batch's mark price in a single batch, expressed as a fraction of `last_mark_price` (default 1%).
