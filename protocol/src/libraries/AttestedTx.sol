...
library AttestedTx {
    ...
    // ---- Struct hash ----
    bytes32 private constant ATTESTED_TX_TYPEHASH =
        keccak256("AttestedTx(bytes32 hash,uint64 committee_epoch)");

    function _hashAttestedTx(AttestedTx memory a) internal pure returns (bytes32) {
        return keccak256(
            abi.encode(
                ATTESTED_TX_TYPEHASH,
                a.hash,
                a.committee_epoch
            )
        );
    ...
    function digest(AttestedTx memory a) public pure returns (bytes32) {
        return keccak256(
            abi.encodePacked("\x19\x01", _domainSeparatorV4(), _hashAttestedTx(a))
        );
    }
}
