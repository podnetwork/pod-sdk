...
library FastTypes {
    ...
    function increment(SharedCounter storage c, bytes32 key, uint256 value) public {
        c._values[key] += value;
    }

    function requireGte(SharedCounter storage c, bytes32 key, uint256 value, string memory errorMessage) public view {
    ...
    function get(OwnedCounter storage c, bytes32 key, address owner) public view returns (uint256) {
        require(owner == tx.origin, "Cannot access OwnedCounter owned by another address");
        return c._values[key][owner];
    }

    function increment(OwnedCounter storage c, bytes32 key, address owner, uint256 value) public {
        require(owner == tx.origin, "Cannot access OwnedCounter owned by another address");
        c._values[key][owner] += value;
    }

    function decrement(OwnedCounter storage c, bytes32 key, address owner, uint256 value) public {
        require(owner == tx.origin, "Cannot access OwnedCounter owned by another address");
        require(c._values[key][owner] >= value, "Cannot decrement counter below 0");
        c._values[key][owner] -= value;
    }

    function set(OwnedCounter storage c, bytes32 key, address owner, uint256 value) public {
        require(owner == tx.origin, "Cannot access OwnedCounter owned by another address");
        c._values[key][owner] = value;
    }
    ...
    function requireGte(Balance storage b, bytes32 key, address owner, uint256 value, string memory errorMessage) public view {
        requireQuorum(b._values[key][owner] >= int256(value), errorMessage);
    }

    function increment(Balance storage b, bytes32 key, address owner, uint256 value) public {
        b._values[key][owner] += int256(value);
    }

    function decrement(Balance storage b, bytes32 key, address owner, uint256 value) public {
    ...
    function add(AddressSet storage s, address value) public {
        add(s._set, bytes32(uint256(uint160(value))));
    }

    function requireExists(AddressSet storage s, address value, string memory errorMessage) public view {
        requireExists(s._set, bytes32(uint256(uint160(value))), errorMessage);
    }

    function requireLengthGte(AddressSet storage s, uint256 value, string memory errorMessage) public view {
        requireLengthGte(s._set, value, errorMessage);
    }
}