use std::{str::FromStr, time::SystemTime, collections::HashSet};

use anyhow::Result;
use clap::{Parser, Subcommand};
use futures::StreamExt;
use pod_sdk::{
    alloy_primitives, alloy_rpc_types::Filter, alloy_sol_types::SolEvent, provider::PodProviderBuilder, Address, EthereumWallet, PrivateKeySigner, TxKind, U256,
};
...
async fn vote(
    ...
    println!("View on explorer: {}/tx/{:?}", POD_EXPLORER_URL, pending_tx.tx_hash());
...
async fn watch(auction_id: u64, deadline: u64, rpc_url: String) -> Result<()> {
    // Validate deadline
    let now = SystemTime::now()
        .duration_since(SystemTime::UNIX_EPOCH)?
        .as_secs();
    ...
    println!("Watching auction {} until deadline {}...", auction_id, deadline);
    println!("View contract on explorer: {}/account/{}", POD_EXPLORER_URL, auction_address);
    ...
    let log_handle = tokio::spawn(async move {
        ...
        let sub = pod_provider_clone.subscribe_verifiable_logs(&filter).await?;
    ...
    let mut deadline_handle = tokio::spawn(async move {
        ...
        pod_provider_clone.wait_past_perfect_time(deadline * 1000000).await?;
        println!("Auction deadline reached!");
        Ok::<_, anyhow::Error>(())
    });
    ...
    if highest_bid > U256::ZERO {
        ...
        println!("View winner on explorer: {}/address/{}", POD_EXPLORER_URL, highest_bidder);
    } else {
